...
// AtMega1284 
#define  CS				PB4		/* Select Slave */ 
#define  MOSI			PB5		/* Master output, slave input */
#define  MISO			PB6		/* Master input, slave output */
#define  SCLK			PB7		/* Clock */
#define  CARD_DETECT	PB1		/* Disk detect, LOG 0 if inserted, need sw pullup */ 
#define  CARD_WP		PB0		/* Disk write lock enable, LOG 0 if write protected */

#define CARD_PWR_ON		DDRB |= (1 << PB2); PORTB &= ~(1 << PB2);		
//#define CARD_PWR_OFF	DDRB |= (1 << PB2);	PORTB |= (1 << PB2);
#define CARD_PWR_OFF CARD_PWR_ON
...
static void power_off(void)
{
	...
	DDRB &= ~((1 << CS) | (1 << SCLK) | (1 << MOSI) | (1 << MISO) | (1 << CARD_DETECT) | (1 << CARD_WP));		    // All to Input
	PORTB &= ~((1 << CS) | (1 << SCLK) | (1 << MOSI) | (1 << MISO));
	PORTB |= ((1 << CARD_DETECT) | (1 << CARD_WP));
...
DSTATUS mmc_disk_initialize(void) {
	...
	for (uint16_t i = 0;; ++i) {
		...
		if (i == 0x1ff) {
			CS_HIGH; //UNSELECT set HIGH (Chip Select)
			return Stat;
		}
	}
	...
	if ((response & (1 << 2)) == 0) {
		xchg_spi(0xff);
		xchg_spi(0xff);
		if ((xchg_spi(0xff) & 0x01) == 0) return Stat; /* card operation voltage range doesn't match */
		if (xchg_spi(0xff) != 0xaa) return Stat; /* wrong test pattern */
	...
	if (ty & CT_SDC2) {
		...
		if (tmp) ty |= (CT_SDC | CT_BLOCK); // ??? is SD_RAW_SPEC_SDHC same as CT_SDC
		//		if((tmp & 0xc0) == 0xc0) printf("SDHC card\r\n");		// SDHC card width
